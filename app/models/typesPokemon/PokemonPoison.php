...
class PokemonPoison extends Pokemon implements Combattant {
    use Soins;
    ...
    {
       parent::__construct($id, $nom, 'Poison', $pointsdevie, $attaque, $defense, $img);
    }
    ...
    public function CapaciteSpeciale(Pokemon $adversaire): int {
        ...
        if ($adversaire->getType() === 'Fee' || $adversaire->getType() === 'Plante') {
            $degatsBase *= self::EFFICACE;
        }
        elseif ($adversaire->getType() === 'Spectre' || $adversaire->getType() === 'Sol' || $adversaire->getType() === 'Roche' || $adversaire->getType() === 'Poison') {
