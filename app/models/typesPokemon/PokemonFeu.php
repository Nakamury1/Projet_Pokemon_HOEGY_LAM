...
class PokemonFeu extends Pokemon implements Combattant {
    use Soins;
    ...
    {
       parent::__construct($id, $nom, 'Feu', $pointsdevie, $attaque, $defense, $img);
    }
    ...
    public function CapaciteSpeciale(Pokemon $adversaire): int {
        ...
        if ($adversaire->getType() === 'Plante' || $adversaire->getType() === 'Glace' || $adversaire->getType() === 'Insecte' || $adversaire->getType() === 'Acier') {
            $degatsBase *= self::EFFICACE;
        }
        elseif ($adversaire->getType() === 'Eau' || $adversaire->getType() === 'Roche' || $adversaire->getType() === 'Dragon' || $adversaire->getType() === 'Feu') {
