...
class PokemonPsy extends Pokemon implements Combattant {
    use Soins;
    ...
    {
       parent::__construct($id, $nom, 'Psy', $pointsdevie, $attaque, $defense, $img);
    }
    ...
    public function CapaciteSpeciale(Pokemon $adversaire): int {
        ...
        if ($adversaire->getType() === 'Combat' || $adversaire->getType() === 'Poison') {
            $degatsBase *= self::FAIBLE;
        }
        elseif ($adversaire->getType() === 'Acier' || $adversaire->getType() === 'Psy') {
            $degatsBase *= self::FAIBLE;
        }
