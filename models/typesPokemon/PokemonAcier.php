...
class PokemonAcier extends Pokemon implements Combattant {
    use Soins;
    ...
    {
       parent::__construct($id, $nom, 'Acier', $pointsdevie, $attaque, $defense, $img);
    }
    ...
    public function CapaciteSpeciale(Pokemon $adversaire): int {
        ...
        if ($adversaire->getType() === 'Glace' || $adversaire->getType() === 'Roche' || $adversaire->getType() === 'Fee') {
            $degatsBase *= self::EFFICACE;
        }
        elseif ($adversaire->getType() === 'Acier' || $adversaire->getType() === 'Feu' || $adversaire->getType() === 'Eau' || $adversaire->getType() === 'Electrik') {
